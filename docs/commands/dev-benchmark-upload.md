...
```
cd $HOME\Desktop
.\sbx.exe dev benchmark upload -num-files NUM -path /DROPBOX/PATH/TO/PROCESS -size-max-kb NUM -size-min-kb NUM
```

macOS, Linux:
```
$HOME/Desktop/sbx dev benchmark upload -num-files NUM -path /DROPBOX/PATH/TO/PROCESS -size-max-kb NUM -size-min-kb NUM
```
