...
```
cd $HOME\Desktop
.\sbx.exe dev release asseturl -source-owner SRC_OWNER -source-repo SRC_REPO -target-owner TARGET_OWNER -target-repo TARGET_REPO -target-branch TARGET_BRANCH
```

macOS, Linux:
```
$HOME/Desktop/sbx dev release asseturl -source-owner SRC_OWNER -source-repo SRC_REPO -target-owner TARGET_OWNER -target-repo TARGET_REPO -target-branch TARGET_BRANCH
```
