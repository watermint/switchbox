...
```
cd $HOME\Desktop
.\sbx.exe dev ci artifact up -dropbox-path /DROPBOX/PATH/TO/UPLOAD -local-path /LOCAL/PATH/OF/ARTIFACT -timeout NUM
```

macOS, Linux:
```
$HOME/Desktop/sbx dev ci artifact up -dropbox-path /DROPBOX/PATH/TO/UPLOAD -local-path /LOCAL/PATH/OF/ARTIFACT -timeout NUM
```
